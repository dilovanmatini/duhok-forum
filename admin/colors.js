...
//////////////////////////////////////////////////////////*/
	...
	function color_palette(ncolor,id){
		var sHTML = "";
                                    var div_id = "admin"+id;
		...
		sHTML += "<table border=0 width=350 align=center cellpadding=0 cellspacing=0>" +
			"<tr>" +
			"<td width=45 align=center bgcolor=ghostwhite style='border:#a9a9a9 1 solid'>" +
			"<table border=1 cellpadding=0 cellspacing=0>" +
			"<tr>" +
			"<td bgcolor='"+ncolor+"' id='"+div_id+"' name='"+div_id+"' width=28 height=28></td>" +
			"</tr>" +
			"</table>" +
			"</td>" +
			"<td>&nbsp;</td>" +
			"<td>"+
			"<table cellpadding=0 cellspacing=1 bgcolor=black>";

		for (var x=1;x<=6;++x){
			sHTML += "<tr>";
			for (var i=0;i<eval("c"+x).length; ++i){
				var color = eval("c"+x+"[i]");
				sHTML += "<td style='cursor:hand;background-color: #"+color+"' onclick=\"switch_color('"+color+"','"+div_id+"','"+id+"');\" width=9 height=6></td>";
			}
			sHTML += "</tr>";
		}
		sHTML += "</table>"+
...
function switch_color(color,id,g){
var n_update = "g"+g;
document.getElementById(id).style.background = '#'+color;
document.getElementById(n_update).value = "#"+color;
}