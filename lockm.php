...
exit();
    ...
    if($Mlevel == 4 OR $Mlevel == 3 AND $CAN_CLOSE_M == 1  AND members("LEVEL",$m) != 4 ){
...
<form method="post" action="index.php?mode=lockm&m='.$m.'&type=add">
<input type="hidden" name="user_id" value="'.$ppMemberID.'">

	<tr class="fixed">
	<td class="cat" colspan="4"><nobr>'.$lang[lock][title].'   '.$Name.'  </td>
	...
	<tr class="fixed">
		<td class="cat"><nobr>'.$lang[lock][why].'</nobr></td>
	<td class="userdetails_data"  colspan="3"><textarea cols="50" rows="5" name="user_cause">'.$ProMemberCause.'</textarea></td>
 	<tr class="fixed">
...
$user_text = HtmlSpecialchars($_POST["user_cause"]);
...
if($error == ""){
        ...
        $query .= " M_STATUS = ('0'), ";
        $query .= " LOCK_DATE = ('$date'), ";
        $query .= " LOCK_BY = ('$DBMemberID'), ";
        $query .= " LOCK_CAUSE = ('$user_text'), ";
