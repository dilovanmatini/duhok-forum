...
DF.doCheckInForum=function(s,sel){
	var frm=s.form;
	if(sel){
		if(s.selectedIndex>0){
			frm.searchForum[1].checked=true;
		}
	}
	else{
		if(s.value=='all'){
			frm.forumId.selectedIndex=0;
		}
	}
};
</script>
...
$searchText=$DF->cleanText($_GET['searchText']);
$searchIn=(($in=$DF->cleanText($_GET['searchIn'])) ? $in : 'title');
$searchForum=(($forum=$DF->cleanText($_GET['searchForum'])) ? $forum : 'all');
$forumId=(int)trim($_GET['forumId']);
...
<table width=\"100%\" cellSpacing=\"0\" cellPadding=\"0\">
	...
	<tr>
		...
		<form method=\"get\" action=\"search.php\">
			<tr>
				...
				<td class=\"asNormalB\"><input type=\"text\" name=\"searchText\" style=\"width:600px\" value=\"$searchText\"></td>
			</tr>
			<tr>
				<td class=\"asFixedB\">بحث في</td>
				<td class=\"asNormalB\">
					<input type=\"radio\" name=\"searchIn\" value=\"title\" {$DF->choose('title',$searchIn,'c')}>&nbsp;عنوان الموضوع&nbsp;&nbsp;&nbsp;&nbsp;
					<input type=\"radio\" name=\"searchIn\" value=\"message\" {$DF->choose('message',$searchIn,'c')}>&nbsp;محتوي الموضوع
				</td>
			</tr>
			<tr>
				<td class=\"asFixedB\">في منتدى</td>
				<td class=\"asNormalB\">
					<input type=\"radio\" name=\"searchForum\" value=\"all\" onclick=\"DF.doCheckInForum(this)\" {$DF->choose('all',$searchForum,'c')}>&nbsp;جميع منتديات&nbsp;&nbsp;&nbsp;&nbsp;
					<input type=\"radio\" name=\"searchForum\" value=\"single\" onclick=\"DF.doCheckInForum(this)\" {$DF->choose('single',$searchForum,'c')}>&nbsp;منتدى
					<select name=\"forumId\" onchange=\"DF.doCheckInForum(this,true)\">
						<option value=\"0\">-- اختر منتدى --</option>";
					foreach($Template->forumsList as $f=>$subject){
						echo"
						<option value=\"$f\" {$DF->choose($f,$forumId,'s')}>$subject</option>";
					}	
					echo"
					</select>
				</td>
	...
	<tr>
		<td class=\"asHeader\">نتائج البحث عن: <span class=\"asC2\">$searchText</span></td>
	</tr>
	<tr>
		<td class=\"asBody\">
...
if(!empty($searchText)){
	if($searchIn=='title'){
		$inSql=" AND t.subject LIKE '%$searchText%'";
	}
	else{
		$inSql=" AND tm.message LIKE '%$searchText%'";
	}
	if($searchForum=='single'&&$forumId>0){
		$forumSql=" AND f.id = $forumId";
	}
	else{
		$forumSql="";
	}
	...
	WHERE t.hidden = 0 AND t.trash = 0 AND t.moderate = 0 {$inSql} {$forumSql} {$checkSqlWhere} ORDER BY t.lpdate DESC LIMIT 50", __FILE__, __LINE__);
	$count=0;
